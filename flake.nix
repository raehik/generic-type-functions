...

{
  ...
  let
    ...
    nondevDevShell = compiler: {
      mkShellArgs.name = "${compiler}-generic-type-functions";
      hoogle = false;
      tools = _: {
        hlint = null;
  ...
  in
    inputs.flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { self', pkgs, config, ... }: {
        packages.default  = self'.packages.ghc96-generic-type-functions;
        ...
        haskellProjects.ghc96 = {
          ...
          devShell.mkShellArgs.name = "ghc96-generic-type-functions";
        };
